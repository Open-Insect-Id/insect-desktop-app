...
def load_model(model_path: str, hierarchy_path: str, labels_path: str):
    ...
    try:
        ...
        logger.info(
            f"Modèle ONNX chargé: input_name={input_name}, output_name={output_name}, input_shape={input_shape}"
        )
    except Exception as e:
        logger.error(f"❌ Erreur modèle: {e}")
        raise

    logger.info(f"Chargement de la hiérarchie depuis: {hierarchy_path}")
    with open(hierarchy_path, "r") as f:
        data = json.load(f)

    hierarchy_map = data["full_taxa_map"]
    logger.info(f"Hiérarchie chargée: {len(hierarchy_map)} entrées")
    ...
    ordre_classes = [
        labels["id_to_name"]["ordre"][str(i)] for i in range(labels["stats"]["ordres"])
    ]
    famille_classes = [
        labels["id_to_name"]["famille"][str(i)]
        for i in range(labels["stats"]["familles"])
    ]
    genre_classes = [
        labels["id_to_name"]["genre"][str(i)] for i in range(labels["stats"]["genres"])
    ]
    sorted_class_to_idx = sorted(labels["class_to_idx"].items(), key=lambda x: x[1])
    espece_classes = [
        full_name.split("_")[-1] for full_name, idx in sorted_class_to_idx
    ]
    ...
    logger.info(
        f"Classes: ordres={len(ordre_classes)}, familles={len(famille_classes)}, genres={len(genre_classes)}, espèces={len(espece_classes)}"
    )
    ...
    logger.info(f"Hiérarchie finale: {len(final_hierarchy)} mappings valides")
    input_size = (
...
def process_image(image_path: str):
    ...
    logger.debug(f"Tensor préparé: shape={input_tensor.shape}")
    ...
    logger.debug(f"Espèce prédite ID: {espece_id}")
    coherent_hierarchy = final_hierarchy.get(espece_id, [0, 0, 0, espece_id])
    logger.debug(f"Hiérarchie cohérente: {coherent_hierarchy}")
    ...
    logger.info(f"Nom d'espèce pour GBIF: {species_name}")
    species_id = gbif_api.get_species_id(species_name)
    info = gbif_api.get_species_info(species_id[0])

    logger.info(f"Résultat final: {names}, confiance moyenne: {avg_conf:.1f}%")
    return {
        "names": names,
