...
from ui.ColorPicker import ColorPickerDialog
...
class SettingsWindow(ctk.CTkToplevel):
    ...
    def _build_ui(self):
        ...
        self.btn_close = ctk.CTkButton(
            top_bar,
            text=self.i18n.t("close") if self.i18n and self.i18n.t("close") else "Fermer",
            width=120,
    ...
    def refresh(self):
        ...
        for row_idx, (key, i18n_key, fallback_label) in enumerate(self._COLOR_KEYS, start=1):
            frame = ctk.CTkFrame(self.scroll, fg_color="transparent")
            frame.grid(row=row_idx, column=0, sticky="ew", pady=6)
            frame.grid_columnconfigure(1, weight=1)

            label_text = (
                self.i18n.t(i18n_key) if self.i18n else fallback_label
            )
            ctk.CTkLabel(frame, text=label_text,
                         font=ctk.CTkFont(size=16), text_color=tc,
            ...
            swatch = tk.Canvas(frame, width=32, height=22,
                               bg=current_color, highlightthickness=1,
    ...
    def _open_picker(self, key: str, current_color: str):
        ...
        dialog = ColorPickerDialog(self, initial_color=current_color, title=picker_title)
